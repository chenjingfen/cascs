...
console.log('mousedown');
    ...
    function mousemove(event) {
        ...
        if (_state === STATE.ROTATE && !_this.noRotate) {
            ...
            var panY = 0;
            var x=0,y=0;
            if (_panEnd.x != Infinity) {
                x=(event.movementX-panX)*1.5;
                panX = event.movementX;

            }
            if (_panEnd.y != Infinity) {
                y=(event.movementY-panY);
                panY = event.movementY ;
            }
            CesMap.viewer.scene.camera.flyTo({
                destination: Cesium.Cartesian3.fromDegrees(center.lon - x, center.lat + y, center.height)
            });
    ...
    function mouseup(event) {
        ...
        _state = STATE.NONE;
        CesMap.viewer.scene.camera.cancelFlight();
    ...
    function mousewheel(event) {
        ...
        if (_this.object.position.z < _this.maxDistance-0.1 && _this.object.position.z > _this.minDistance+0.1) {
            if (delta > 0) {
                CesMap.viewer.scene.camera.zoomIn();
            } else {
                CesMap.viewer.scene.camera.zoomOut();
            }
        }
        _this.dispatchEvent(startEvent);
        _this.dispatchEvent(endEvent);
